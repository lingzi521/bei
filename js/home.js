	...
	'Login':{
		...
		'Create':function($id){
			...
			if(MAC.Cookie.Get('userid')!=undefined && MAC.Cookie.Get('userid')!=''){ 
				html+='<div class="login-buttons"><a href="/?m=user-logout-flag-main.html"> <input type="submit" style="outline:none;" value="退出" class="btn btn-success btn-block btn-lg"><a></div>';
			}
			else{
				html+='<form class="new_user" onsubmit="return false;" id="loginform_web" action="/index.php?user-check" accept-charset="UTF-8" method="post"><input id="flag" name="flag" type="hidden" value="&#x2713;"><div class="alert alert-danger">注：请正确输入权限令牌（口令/密令）</div><div class="form-group m-b-15"><input autofocus class="form-control input-lg" placeholder="口令" type="text" value="" id="username" name="username"></div><div class="form-group m-b-15"><input autocomplete="off" class="form-control input-lg" placeholder="密令" type="password" id="userpass" name="userpass"></div><div class="checkbox m-b-30"><label><input type="checkbox" name="remember" id="remember" checked value="1"> 记住登录信息</label></div><div class="login-buttons"><input type="submit" style="outline:none;" value="确定" id="loginbtn" class="btn btn-success btn-block btn-lg"></div><div class="m-t-20 m-b-20 text-inverse pull-left">一年权限？ <a>暂时没有</a></div><div class="m-t-20 m-b-40 p-b-40 text-inverse pull-right">三年权限？ <a class="text-success" href="/template/paody/tab/zhifu.html">点此获取</a></div></form>';
			}
		...
		'In':function(){
			if($("#username").val()==''){ alert('请输入口令！'); return ;}
			if($("#userpass").val()==''){ alert('请输入密令！'); return;}
			$.ajax({type: 'post',url: SitePath + 'index.php?m=user-check', data:'u_name='+$("#username").val()+'&u_password='+$("#userpass").val(),timeout: 5000,
				...
				success: function($r){
					if($r.indexOf('您输入')>-1){
						...
						location.href='/?m=vod-type-id-1.html';
					}
					else{
